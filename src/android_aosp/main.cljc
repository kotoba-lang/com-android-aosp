(ns android_aosp.main
  "Kotodama WASM entrypoint for the Android_aosp clean-room actor (L4) — Clojure port.
  ...
  facts (`android_aosp.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "android_aosp")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"android_aosp.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "android_aosp-compat" :tier tier :entities entities} 200])
...
(defn start! [] :android_aosp-compat/ready)
